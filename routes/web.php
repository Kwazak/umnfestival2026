...
Route::get('/event/unveiling', function () {
    return inertia('EventUnveiling');
});

// Additional event pages
Route::get('/event/eulympic', function () {
    return inertia('EventEulympic');
});
Route::get('/event/ucare', function () {
    return inertia('EventUcare');
});
Route::get('/event/ulympic', function () {
    return inertia('EventUlympic');
});
Route::get('/event/unify', function () {
    return inertia('EventUnify');
});
...
// Vanguards Leaderboard Routes (Password Protected)
Route::get('/vanguards/login', [App\Http\Controllers\LeaderboardController::class, 'showLogin'])->name('vanguards.login');
Route::post('/vanguards/login', [App\Http\Controllers\LeaderboardController::class, 'authenticate'])->name('vanguards.authenticate');
Route::get('/vanguards/leaderboard', [App\Http\Controllers\LeaderboardController::class, 'index'])->name('vanguards.leaderboard');
Route::post('/vanguards/logout', [App\Http\Controllers\LeaderboardController::class, 'logout'])->name('vanguards.logout');
...
Route::prefix('admin')->middleware('admin.auth')->group(function () {
    ...
    Route::get('/discount-codes', [App\Http\Controllers\AdminController::class, 'discountCodes'])->name('admin.discount-codes');
    ...
    Route::get('/merchandise', [App\Http\Controllers\AdminController::class, 'merchandise'])->name('admin.merchandise');
    Route::get('/countdown', [App\Http\Controllers\AdminController::class, 'countdown'])->name('admin.countdown');
    ...
    Route::get('/chatbot-training', [App\Http\Controllers\AdminController::class, 'chatbotTraining'])->name('admin.chatbot-training');
    Route::get('/email-blast', [App\Http\Controllers\AdminController::class, 'emailBlast'])->name('admin.email-blast');
    ...
    // Scanner Routes (ORANG-1 to ORANG-50) - Protected by admin.auth
    Route::get('/scanner/login', function () {
        return inertia('ScannerLogin');
    })->name('scanner.login');
    Route::post('/scanner/login', [App\Http\Controllers\API\TicketController::class, 'scannerLogin'])->name('scanner.login.post');
    Route::post('/scanner/logout', [App\Http\Controllers\API\TicketController::class, 'scannerLogout'])->name('scanner.logout');
});
...
// Midtrans notification webhook (no API prefix fallback)
Route::post('/payment/notification', [ApiPaymentController::class, 'notification'])
