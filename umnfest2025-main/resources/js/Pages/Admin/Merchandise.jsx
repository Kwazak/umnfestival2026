...
const Merchandise = () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                for (let i = 0; i < data.data.length; i++) {
                    try {
                        const detailResponse = await fetch(`/api/event-upcoming-details/${i + 1}`);
                        ...
                        if (detailData.success) {
                            ...
                            adminEvents.push({
                                ...
                                image_url: data.data[i].image,
                                alt_text: data.data[i].alt,
                                is_locked: data.data[i].isLocked,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editingEvent 
                ? `/api/event-upcoming-details/${editingEvent.id}`
                : '/api/event-upcoming-details';
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`/api/event-upcoming-details/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                }
        ...
        <AdminLayout>
            <div className="container mx-auto px-4 py-8">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h1 className="text-3xl font-bold">Event Upcoming Details Management</h1>
                        <p className="text-gray-600 mt-1">Manage events displayed in the merchandise section</p>
                    </div>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                {events.map((event) => (
                                    <tr key={event.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {event.image_url ? (
                                                <img src={event.image_url} alt={event.alt_text} className="h-10 w-10 object-cover rounded" />
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                                ...
                                                {event.is_locked ? 'Locked' : 'Available'}
                                            </span>
                                        </td>
